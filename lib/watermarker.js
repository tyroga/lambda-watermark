...
Watermarker.prototype.watermark = function(image, next) {
  ...
  if (
    image.ContentType.toLowerCase() !== 'image/png' &&
...
Watermarker.prototype._resizeWatermarkImage = function(image, next) {
  ...
  //console.log(Object.keys((image.Body)));
  ...
  var watermarkWidth = this.options.watermarkWidth;
  var watermarkHeight = this.options.watermarkHeight;
  gm(image.Body).size(function(err, size) {
    var watermarkSide = Math.sqrt(
      watermarkWidth * watermarkHeight * (relativeSize / 100)
    )
